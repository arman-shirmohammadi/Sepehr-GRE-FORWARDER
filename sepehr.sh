    ...
    while true; do
        ...
        if [[ "$raw" =~ ^[0-9]+$ ]]; then
            ...
            if valid_port "$s" && valid_port "$e" && ((s<=e)); then
                local p; for ((p=s; p<=e; p++)); do ports+=("$p"); done
            else ok=0; fi
        elif [[ "$raw" =~ ^[0-9]+(,[0-9]+)+$ ]]; then
...
show_unit_status_brief() { systemctl --no-pager --full status "$1" 2>&1 | sed -n '1,12p'; }
...
# ──────────────────────────────────────────────
#          تابع جدید: اضافه کردن کرون‌جاب
# ──────────────────────────────────────────────
add_cron_restart() {
    local unit="$1"
    local cron_file="/etc/cron.d/gre_restart_${unit%.service}"
    
    if [[ -f "$cron_file" ]]; then
        add_log "Cronjob already exists for $unit → $cron_file"
        return 0
    fi

    add_log "Adding cronjob for $unit → every 10 minutes restart"
    
    cat > "$cron_file" <<EOF
# Auto-generated by GRE script - restart $unit every 10 minutes
*/10 * * * * root systemctl restart $unit >/dev/null 2>&1 || true
EOF

    chmod 644 "$cron_file"
    add_log "Cronjob created: $cron_file"
    add_log "Service $unit will restart every 10 minutes"
}
...
uninstall_clean() {
    ...
    add_log "Removing related cronjobs if exist..."
    rm -f "/etc/cron.d/gre_restart_gre${id}" >/dev/null 2>&1 || true
    rm -f "/etc/cron.d/gre_restart_fw-gre${id}-"* >/dev/null 2>&1 || true
    add_log "Reloading systemd..."
